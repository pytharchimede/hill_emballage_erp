...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'entry_create') {
            ...
            // Sécurité dépôt: vérifier que l'utilisateur est autorisé à agir sur ce dépôt
            $uRole = $_SESSION['user_role'] ?? '';
            $uDepot = (int)($_SESSION['depot_id'] ?? 0);
            $uIsMain = $uDepot > 0 && isMainDepot($uDepot);
            $okDepot = false;
            if ($uRole === 'admin' && $uIsMain) {
                $okDepot = true; // admin dépôt principal: tous dépôts
            } else {
                $okDepot = ($depot_id === $uDepot);
            }
