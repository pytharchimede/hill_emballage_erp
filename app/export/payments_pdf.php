...
$st = $db->prepare("SELECT p.*, v.numero_vente, c.nom as client_nom FROM payments p JOIN ventes v ON p.vente_id=v.id JOIN clients c ON v.client_id=c.id $w ORDER BY p.created_at DESC");
$st->execute($p);
